  ...
  | instr :: code' ->
    let env, asm = 
      ...
      | BEGIN (fName, params, locals) -> 
        let pushRegs = List.map (fun x -> Push (R x)) (initList num_of_regs (fun x -> x)) in
        let env = env#enter fName params locals in
        env, [Push ebp; Mov (esp, ebp)] @ pushRegs @ [Binop ("-", M ("$" ^ env#lsize), esp)]
      | END ->
        let popRegs = List.map (fun x -> Pop (R x)) (List.rev (initList num_of_regs (fun x -> x))) in 
...
let make_assoc l = List.combine l (initList (List.length l) (fun x -> x))
